...
namespace Another_YW_4_Save_Editor
{
    public partial class Form1 : Form
    {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    using (openedFile = openFileDialog1.OpenFile())
                    {
                        ...
                        foreach (Equipment equip in saveFileParams.EquipmentList)
                        {
                            equipListView.Items.Add(equip.ID1.ToString()).SubItems.AddRange(new string[] {
                                equip.ID2.ToString(),
                                equip.Order.ToString(),
                                new GetEquipment().pickEquipmentName(equip.ItemSignature ?? "Invalid"),
                        ...
                        equipListView.Items[0].Selected = true;
        ...
        private void replaceEquipBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                saveFileParams.EquipmentList[item.Index].ItemSignature = new SetEquipment().pickBytesFromIndex(equipCbox.SelectedIndex);
                saveFileParams.EquipmentList[item.Index].Quantity = Convert.ToInt32(equipQtNbox.Value);

                if (saveFileParams.EquipmentList[item.Index].ID2 == 0)
                {
                    saveFileParams.EquipmentList[item.Index].ID1 = item.Index + 4096;
                    for (int i = 1; i < 500; i++)
                    {
                        bool equipIdExist = false;
                        foreach (Equipment equip in saveFileParams.EquipmentList)
                        {
                            if (i == equip.ID2) { equipIdExist = true; break; }
                        }
                        if (!equipIdExist)
                        {
                            saveFileParams.EquipmentList[item.Index].ID2 = i;
                            saveFileParams.EquipmentList[item.Index].Order = i; break;
                        };
                    }
                }
                ...
                item.SubItems.Clear(); item.SubItems.AddRange(new string[]
                {
                    saveFileParams.EquipmentList[item.Index].ID2.ToString(),
                    saveFileParams.EquipmentList[item.Index].Order.ToString(),
                    new GetEquipment().pickEquipmentName(saveFileParams.EquipmentList[item.Index].ItemSignature ?? "Invalid"),
                    saveFileParams.EquipmentList[item.Index].Quantity.ToString()
                });
                item.Text = saveFileParams.EquipmentList[item.Index].ID1.ToString();
            }
        }
        ...
        private void removeEquipBtn_Click(object sender, EventArgs e)
        {
            foreach (ListViewItem item in equipListView.SelectedItems)
            {
                saveFileParams.EquipmentList[item.Index].ID1 = 0;
                saveFileParams.EquipmentList[item.Index].ID2 = 0;
                saveFileParams.EquipmentList[item.Index].Order = 0;
                saveFileParams.EquipmentList[item.Index].ItemSignature = "00-00-00-00";
                saveFileParams.EquipmentList[item.Index].Quantity = 0;
        ...
        private void equipListView_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (ListViewItem item in equipListView.SelectedItems)
            {
                equipCbox.SelectedIndex = new GetEquipment().pickEquipmentIndex(saveFileParams.EquipmentList[item.Index].ItemSignature ?? "");
                equipQtNbox.Value = saveFileParams.EquipmentList[item.Index].Quantity;
            }
        }
